...
// Type of the result is affected by the specified include parameter.
...
const members = group.relations
  ...
  // Extract the target artist. The `artist` property is guaranteed to exist and
  // not optional because "artist-rels" is the only relationship include.
  .map((rel) => rel.artist);
